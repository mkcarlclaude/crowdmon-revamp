...
func Load() (Config, error) {
	cfg := Config{
		...
		OTLPMetricsEndpoint: os.Getenv("CROWDMON_OTLP_METRICS_ENDPOINT"),
	...
	// omitting these is spans (or log records, or metrics) that never arrive,
	// noticed days later.
	//
	// All three signals share one token and one Access application (CONTEXT.md
	// §6), so this is one check rather than three: adding a fourth signal means
	// adding its predicate here, and forgetting to is the failure this shape
	// makes obvious.
	if (cfg.TracingEnabled() || cfg.LogsEnabled() || cfg.MetricsEnabled()) &&
		(cfg.AccessClientID == "" || cfg.AccessClientSecret == "") {
		return Config{}, fmt.Errorf(
			"CROWDMON_OTLP_ENDPOINT, CROWDMON_OTLP_LOGS_ENDPOINT or " +
				"CROWDMON_OTLP_METRICS_ENDPOINT is set, so CF_ACCESS_CLIENT_ID " +
				"and CF_ACCESS_CLIENT_SECRET are required")
	}
