...
type ClientInterface interface {
	...
	//
	// The whole unruled pool, not the curated public sample — see this file's module comment for why. Restricted to the train split (M26.6, plan §A): a frame with `selection_reason = 'random'` never appears here, on the forward page or the wrap, because that split is the permanent evaluation pool (CONTEXT.md §Q16) and a verdict on it trains nothing. A frame is returned while any of its boxes carries neither an admin verdict nor a trusted user's, and carries only those boxes. Shuffled by a per-image random key, not extraction order (M25.1); pass `cursor` back as `next_cursor` came from the previous call to keep advancing, and omit it to start over — the pool wraps rather than running dry once a cursor passes every key still in it. `remaining` is capped rather than exact past 500, since this route is public and unauthenticated. Requires a contributor session.
	//
